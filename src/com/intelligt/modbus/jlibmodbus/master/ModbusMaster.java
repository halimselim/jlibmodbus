...
abstract public class ModbusMaster implements FrameEventListenerList {
    ...
    /**
     * this method allows you to implement your own behavior of disconnect
     * method.
     *
     * @throws ModbusIOException
     */
    ...
    /**
     * this function allows you to process your own ModbusRequest. Each request
     * class has a compliant response class for instance
     * ReadHoldingRegistersRequest and ReadHoldingRegistersResponse. If you
     * process an instance of ReadHoldingRegistersRequest this function exactly
     * either returns a ReadHoldingRegistersResponse instance or throws an
     * exception.
     *
    ...
    /**
     * ModbusMaster will block for only this amount of time. If the timeout
     * expires, a ModbusTransportException is raised, though the ModbusMaster is
     * still valid.
     *
...
//    final public int[] readInputRegisters(int serverAddress, int startAddress, int quantity) throws
    final public ModbusHoldingRegisters readInputRegisters(int serverAddress, int startAddress, int quantity) throws
            ModbusProtocolException, ModbusNumberException, ModbusIOException {
...
//        return response.getRegisters();
     ...
     * the PDU Coils are addressed starting at zero. Therefore coils numbered
     * 1-16 are addressed as 0-15. If the returned output quantity is not a
     * multiple of eight, the remaining coils in the final boolean array will be
     * padded with FALSE.
     *
     * @param serverAddress a slave address
     ...
     * zero. Therefore Discrete inputs numbered 1-16 are addressed as 0-15. If
     * the returned input quantity is not a multiple of eight, the remaining
     * inputs in the final boolean array will be padded with FALSE.
     *
     * @param serverAddress a slave address
     ...
     * in the request data field. A value of TRUE requests the output to be ON.
     * A value of FALSE requests it to be OFF. The Request PDU specifies the
     ...
     * specified by contents of the request array of boolean. A logical 'true'
     * in position of the array requests the corresponding output to be ON. A
     * logical 'false' requests it to be OFF.
     *
     * @param serverAddress a slave address
     ...
     * registers, and the data to be written.
     *
     * @param serverAddress a slave address
     ...
     * (FIFO) queue of register in a remote device. The function returns the
     * queued data. Up to 31 queue data registers can be read. The function
     * reads the queue contents, but does not clear them. If the queue count
     * exceeds 31, an exception response is returned with an error code of 03
     * (Illegal Data Value).
     *
     * @param serverAddress a slave address
    ...
    /**
     * This function code is used to perform a file record read. A file is an
     * organization of records. Each file contains 10000 records, addressed 0000
     * to 9999 decimal or 0X0000 to 0X270F. For example, record 12 is addressed
     * as 12. The function can read multiple groups of references.
     *
     * @param serverAddress a slave address
     ...
     * Lengths are provided in terms of the number of 16-bit words. A file is an
     * organization of records. Each file contains 10000 records, addressed 0000
     * to 9999 decimal or 0X0000 to 0X270F. For example, record 12 is addressed
     * as 12. The function can write multiple groups of references.
     *
     ...
     * And_Mask)) For example: Hex Binary Current Contents= 12 0001 0010
     * And_Mask = F2 1111 0010 Or_Mask = 25 0010 0101
     * <p>
     * (NOT And_Mask)= 0D 0000 1101
     * <p>
     * Result = 17 0001 0111
     * <p>
     * Note: y If the Or_Mask value is zero, the result is simply the logical
     * ANDing of the current contents and And_Mask. If the And_Mask value is
     * zero, the result is equal to the Or_Mask value. y The contents of the
     * register can be read with the Read Holding Registers function (function
     * code 03). They could, however, be changed subsequently as the controller
     ...
     * known (no output reference is needed in the function). The normal
     * response contains the status of the eight Exception Status outputs. The
     * outputs are packed into one data byte, with one bit per output. The
     * status of the lowest output reference is contained in the least
     * significant bit of the byte. The contents of the eight Exception Status
     * outputs are device specific.
     *
     * @param serverAddress a slave address
     ...
     * current status, and other information specific to a remote device. The
     ...
     * the remote device's communication event counter. By fetching the current
     * count before and after a series of messages, a client can determine
     * whether the messages were handled normally by the remote device. The
     * device’s event counter is incremented once for each successful message
     * completion. It is not incremented for exception responses, poll commands,
     * or fetch event counter commands. The event counter can be reset by means
     * of the Diagnostics function (code 08), with a subfunction of Restart
     * Communications Option (code 00 01) or Clear Counters and Diagnostic
     * Register (code 00 0A).
     *
     * @param serverAddress a slave address
     ...
     * count, and a field of event bytes from the remote device. The status word
     * and event counts are identical to that returned by the Get Communications
     * Event Counter function (11, 0B hex). The message counter contains the
     * quantity of messages processed by the remote device since its last
     * restart, clear counters operation, or power–up. This count is identical
     * to that returned by the Diagnostic function (code 08), sub-function
     * Return Bus Message Count (code 11, 0B hex). The event bytes field
     * contains 0-64 bytes, with each byte corresponding to the status of one
     * MODBUS send or receive operation for the remote device. The remote device
     * enters the events into the field in chronological order. Byte 0 is the
     * most recent event. Each new byte flushes the oldest byte from the field.
     *
     * @param serverAddress a slave address
     * @return the CommStatus instance
     * @throws ModbusProtocolException if modbus-exception is received
     ...
     * occurs before the restart is executed. When the remote device receives
     * the request, it attempts a restart and executes its power–up confidence
     * tests. Successful completion of the tests will bring the port online. A
     * request data field contents of FF 00 hex causes the port’s Communications
    ...
    /**
     * Returns the contents of the remote device’s 16–bit diagnostic register
    ...
    /**
     * The character passed in the request data field becomes the end of message
     * delimiter for future messages (replacing the default LF character). This
     * function is useful in cases of a Line Feed is not required at the end of
     * ASCII messages.
     *
     * @param serverAddress a slave address
     ...
     * addressed remote device. No response is returned. When the remote device
     * enters its Listen Only Mode, all active communication controls are turned
     * off. The Ready watchdog timer is allowed to expire, locking the controls
     * off. While the device is in this mode, any MODBUS messages addressed to
     * it or broadcast are monitored, but no actions will be taken and no
     * responses will be sent. The only function that will be processed after
     * the mode is entered will be the Restart Communications Option function
     * (function code 8, sub-function 1).
     *
     * @param serverAddress a slave address
     * @throws ModbusNumberException if server address is in-valid
     */
    ...
    /**
     * The response data field returns the quantity of CRC errors encountered by
     * the remote device since its last restart, clear counters operation, or
     * power–up.
     *
     * @param serverAddress a slave address
     ...
     * responses returned by the remote device since its last restart, clear
    ...
    /**
     * The response data field returns the quantity of messages addressed to the
     * remote device for which it has returned no response (neither a normal
    ...
    /**
     * The response data field returns the quantity of messages addressed to the
     * remote device for which it returned a Negative Acknowledge (NAK)
     * exception response, since its last restart, clear counters operation, or
     * power–up.
     *
     * @param serverAddress a slave address
     * @return NAK count
    ...
    /**
     * The response data field returns the quantity of messages addressed to the
     * remote device for which it returned a Slave Device Busy exception
    ...
    /**
     * The response data field returns the quantity of messages addressed to the
     * remote device that it could not handle due to a character overrun
     * condition, since its last restart, clear counters operation, or power–up.
     * A character overrun is caused by data characters arriving at the port
     ...
     * remote device, only. The Read Device Identification interface is modeled
     * as an address space composed of a set of addressable data elements. The
     * data elements are called objects and an object Id identifies them. The
     * interface consists of 3 categories of objects :  Basic Device
     * Identification. All objects of this category are mandatory : VendorName,
     * Product code, and revision number.  Regular Device Identification. In
     * addition to Basic data objects, the device provides additional and
     * optional identification and description data objects. All of the objects
     * of this category are defined in the standard but their implementation is
     * optional .  Extended Device Identification. In addition to regular data
     * objects, the device provides additional and optional identification and
     * description private data about the physical device itself. All of these
     * data are device dependent. ObjectId ObjectName/Description Type M/O
     * category 0x00 VendorName ASCII String Mandatory Basic 0x01 ProductCode
     * ASCII String Mandatory Basic 0x02 MajorMinorRevision ASCII String
     * Mandatory Basic 0x03 VendorUrl ASCII String Optional Regular 0x04
     * ProductName ASCII String Optional Regular 0x05 ModelName ASCII String
     * Optional Regular 0x06 UserApplicationName ASCII String Optional Regular
     * 0x07 Reserved Optional Optional Regular … 0x7F 0x80 Private objects may
     * be optionally device Optional Extended … defined. The range [0x80 – 0xFF]
     * dependant 0xFF is Product dependant.
     */
